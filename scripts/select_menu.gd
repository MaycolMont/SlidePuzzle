...
@onready var button_size : Button = $VBoxContainer2/SizeSelector/Button
...
func _set_options(options_container, path, button_group):
	...
	file_names = _filter_files(file_names, 'svg')
	var textures_resources = _get_texture_resources(file_names)
	_generate_buttons(options_container, textures_resources, button_group)

func _generate_buttons(buttons_container, texture_resources : Array, button_group):
	for texture_resource in texture_resources:
		var button = _create_button(button_group, texture_resource)
...
func _filter_files(list_files: Array, type: String):
	var files_path = []
...
func _on_play_button_pressed():
	...
	Global.texture_resource = button_image_pressed.icon
